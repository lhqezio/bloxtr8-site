...
type SafariMediaType = "auto" | "image" | "video" | "youtube"
...
export interface SafariProps extends HTMLAttributes<HTMLDivElement> {
  ...
  videoSrc?: string
  /**
   * Optional single-source media prop.
   * - YouTube URL -> embedded iframe
   * - Local/remote video file (mp4/webm/ogg/mov) -> <video>
   * - Image file (png/jpg/webp/gif/svg) -> <img>
   *
   * If provided, this takes precedence over `imageSrc`/`videoSrc`.
   */
  mediaSrc?: string
  /** Force how to render `mediaSrc`. Default: "auto". */
  mediaType?: SafariMediaType
  mode?: SafariMode
}

function getYouTubeEmbedUrl(inputUrl: string) {
  try {
...
function looksLikeVideoFile(src: string) {
...
function looksLikeImageFile(src: string) {
  const clean = src.split("?")[0].split("#")[0].toLowerCase()
  return (
    clean.endsWith(".png") ||
    clean.endsWith(".jpg") ||
...
export function Safari({
  ...
  const resolvedMediaSrc = mediaSrc ?? videoSrc ?? imageSrc

  const resolvedYoutubeEmbedUrl =
    resolvedMediaSrc && (mediaType === "youtube" || mediaType === "auto")
      ? getYouTubeEmbedUrl(resolvedMediaSrc)
      : null

  const resolvedIsYoutube = !!resolvedYoutubeEmbedUrl
  const resolvedIsVideo =
    !!resolvedMediaSrc &&
    (mediaType === "video" ||
      (mediaType === "auto" && !resolvedIsYoutube && looksLikeVideoFile(resolvedMediaSrc)))
  const resolvedIsImage =
    !!resolvedMediaSrc &&
    (mediaType === "image" ||
      (mediaType === "auto" && !resolvedIsYoutube && !resolvedIsVideo && looksLikeImageFile(resolvedMediaSrc)))

  const hasMedia = resolvedIsYoutube || resolvedIsVideo || resolvedIsImage
  ...
  return (
    ...
    >
      {(resolvedIsYoutube || resolvedIsVideo) && (
        ...
        >
          {resolvedIsYoutube && resolvedYoutubeEmbedUrl ? (
            <iframe
